! function() {
    ...
    function goodslist(id, count) {
        ...
        // console.log(count)
        $.ajax({
            url: 'http://10.31.163.13/JS1902/projectname/php/indexone.php', //获取所有的接口数据
            dataType: 'json'
        }).done(function(data) {
            //console.log(data)
            $.each(data, function(index, value) {
                // console.log(value)
                if (id == value.pid) { //遍历判断sid和传入的sid是否相同，方便将那条数据设置到渲染的商品列表中。
                    //console.log(value)
                    var $clonebox = $('.goods_list').clone(true, true);
                    $clonebox.find('.goods-pic').attr('src', value.url);
                    $clonebox.find('.goods-pic').attr('sid', value.pid);
                    $clonebox.find('.goods-pic').html(value.titile);
                    $clonebox.find('.b-price').html(value.price);
                    $clonebox.find('.num_').html(count);
                    //计算每个商品的价格。
                    $clonebox.find('.b-sum').find('span').html((value.price * count).toFixed(2));
                    $clonebox.css('display', 'block');
                    $('table').append($clonebox);
                    priceall(); //计算总价的。
                }
            });
        })
    ...
    function priceall() {
        var $sum = 0;
        var $count = 0;
        $('.goods_list:visible').each(function(index, element) {
            //console.log(element)
            if ($(element).find('.cart-checkbox input').prop('checked')) {
                console.log(element)
                $sum += parseInt($(element).find('.shuliang').val());
                //console.log($sum)
                $count += parseFloat($(element).find('.b-sum').find('span').html());
            }
        });
        $('.bar').find('em').html($sum);
        $('.money').find('em').html($count.toFixed(2));
    }
    //5.全选操作
    $('.allsel').on('change', function() {
        $('.goods_list:visible').find(':checkbox').prop('checked', $(this).prop('checked'));
        $('.allsel').prop('checked', $(this).prop('checked'));
        priceall(); //取消选项，重算总和。
    });

    var $inputs = $('.goods_list:visible').find(':checkbox');
    $('.item-list').on('change', $inputs, function() { //事件的委托的this指向被委托的元素
        if ($('.goods_list:visible').find('input:checkbox').length == $('.goods_list:visible').find('input:checked').size()) {
